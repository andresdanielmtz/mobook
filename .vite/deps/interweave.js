...
var ALLOWED_TAG_LIST = Object.keys(TAGS).filter(
...
var Matcher = class {
  ...
  createElement(children, props) {
    const element = this.factory
      ? import_react.default.createElement(this.factory, props, children)
      : this.replaceWith(children, props);
    if (
      typeof element !== "string" &&
      !import_react.default.isValidElement(element)
    ) {
      throw new Error(
        `Invalid React element created from ${this.constructor.name}.`,
      );
    }
    return element;
  }
...
var Parser = class {
  constructor(markup, props = {}, matchers = [], filters = []) {
    ...
    this.allowed = new Set(
      (_props$allowList = props.allowList) !== null &&
      _props$allowList !== void 0
        ? _props$allowList
        : ALLOWED_TAG_LIST,
    );
    ...
    this.matchers.forEach((matcher) => {
      ...
      while (matchedString && (parts = matcher.match(matchedString))) {
        ...
        if (matcher.greedy) {
          ...
          matchedString = matchedString.slice(
            index + (length || match2.length),
          );
        }
      }
      if (!matcher.greedy) {
  ...
  canRenderChild(parentConfig, childConfig) {
    ...
    if (
      parentConfig.invalid.length > 0 &&
      parentConfig.invalid.includes(childConfig.tagName)
    ) {
      return false;
    }
  ...
  createContainer(markup) {
    ...
    const factory =
      (typeof global !== "undefined" && global.INTERWEAVE_SSR_POLYFILL) ||
      createDocument;
    const doc = factory();
    if (!doc) {
      return void 0;
    ...
    const tag =
      (_this$props$container = this.props.containerTagName) !== null &&
      _this$props$container !== void 0
        ? _this$props$container
        : "body";
    const el =
      tag === "body" || tag === "fragment" ? doc.body : doc.createElement(tag);
    if (markup.match(INVALID_ROOTS)) {
      if (true) {
        throw new Error(
          "HTML documents as Interweave content are not supported.",
        );
      }
    } else {
      el.innerHTML = this.convertLineBreaks(
        this.props.escapeHtml
          ? (0, import_escape_html.default)(markup)
          : markup,
      );
    }
    return el;
  ...
  extractAttributes(node) {
    ...
    [...node.attributes].forEach((attr) => {
      ...
      let newValue =
        newName === "style" ? this.extractStyleAttribute(node) : value;
      ...
      attributes[ATTRIBUTES_TO_PROPS[newName] || newName] =
        this.applyAttributeFilters(newName, newValue);
      count += 1;
    });
    if (count === 0) {
      return null;
  ...
  extractStyleAttribute(node) {
    ...
    Array.from(node.style).forEach((key) => {
      ...
      if (typeof value === "string" || typeof value === "number") {
        styles[
          key.replace(/-([a-z])/g, (match2, letter) =>
  ...
  parseNode(parentNode, parentConfig) {
    ...
    [...parentNode.childNodes].forEach((node) => {
      if (node.nodeType === ELEMENT_NODE) {
        ...
        if (
          transform &&
          !(transformOnlyAllowList && !this.isTagAllowed(tagName))
        ) {
          this.keyIndex += 1;
          ...
          if (typeof transformed !== "undefined") {
            content.push(
              import_react2.default.cloneElement(transformed, {
                key,
              }),
            );
            return;
        ...
        if (
          ...
          content.push(
            import_react2.default.createElement(
              ...
              (_children = children) !== null && _children !== void 0
                ? _children
  ...
  replaceTokens(tokenizedString, elements) {
    ...
    while ((open = text.match(OPEN_TOKEN))) {
      ...
      if (!elements[tokenName]) {
        throw new Error(
          `Token "${tokenName}" found but no matching element to replace with.`,
        );
      }
      if (startIndex > 0) {
      ...
      if (isVoid) {
        ...
        if (!close) {
          throw new Error(
            `Closing token missing for interpolated element "${tokenName}".`,
          );
        }
        ...
        nodes.push(
          matcher.createElement(
            this.replaceTokens(
              text.slice(match2.length, close.index),
              elements,
            ),
            elementProps,
          ),
...
function Markup(props) {
  ...
  const tag =
    (_ref =
      containerTagName !== null && containerTagName !== void 0
        ? containerTagName
        : tagName) !== null && _ref !== void 0
      ? _ref
      : "span";
...
function Interweave(props) {
  ...
  const markup = beforeCallbacks.reduce(
    (string, callback) => {
      const nextString = callback(string, props);
      if (typeof nextString !== "string") {
        throw new TypeError(
          "Interweave `onBeforeParse` must return a valid HTML string.",
        );
      }
      return nextString;
    },
